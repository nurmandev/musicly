                                ...
                                <div className="col-xl-4 col-md-6" key={item.id}>
                                    <div className="ms-news-item p-relative zindex-1 mb-40">
                                        ...
                                        <div className="ms-news-content ms-news-position p-absolute">
                                            <Link className="ms-news2-cat mb-30" href={`/news-details/${item.id}`}>{item.category}</Link>
                                            <h3 className="ms-news-title mb-15"><Link href={`/news-details/${item.id}`}>{item.title}</Link></h3>
                                            <div className="ms-news-meta d-inline-block">
                                                <span>{item.date}</span>
                                                <span><Link href={`/news-details/${item.id}`}>{item.comment} Comments</Link></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
